            ...
            <button
              onClick={async () => {
                ...
                try {
                  ...
                  console.log("📋 Checking appointments:", appointments.length);
        ...
        >
          <div className="flex items-start justify-between">
            <div className="flex-1">
              ...
              >
                Create user records from chat participants. Perfect for fixing
                missing patient names in chats.
              </p>
            </div>
            <button
              onClick={async () => {
                ...
                try {
                  ...
                  // Check each chat for patientId
                  for (const chatDoc of chatsSnapshot.docs) {
                    const chatData = chatDoc.data();
                    const patientId = chatData.patientId;

                    if (patientId && !existingUserIds.has(patientId)) {
                      console.log(`Found chat patient: ${patientId}`);

                      // Try to get info from chat messages
                      const messagesSnapshot = await getDocs(
                        collection(db, "chats", chatDoc.id, "messages")
                      );

                      let patientEmail = chatData.patientEmail || "";
                      let patientName = chatData.patientName || "Patient";

                      // Try to get better info from messages
                      messagesSnapshot.docs.forEach((msgDoc) => {
                        ...
                        if (
                          ...
                          if (msg.senderEmail) patientEmail = msg.senderEmail;
                          if (msg.senderName && msg.senderName !== "Patient") {
                            patientName = msg.senderName;
                          }
                        }
                      });

                      usersToCreate.set(patientId, {
                        id: patientId,
                        email: patientEmail || `${patientId}@temporary.com`,
                        displayName:
                          patientName !== "Patient"
                            ? patientName
                  ...
                  // Create the users
                  let syncedCount = 0;
                  for (const [patientId, userData] of usersToCreate) {
                    console.log(
                      `Creating user from chat: ${patientId}, name: ${userData.displayName}`
                    );
                  ...
                  setMessage({
                    type: "success",
                    text: `✅ Successfully synced ${syncedCount} users from chats!`,
                  });
                  ...
                  console.error("Error syncing from chats:", error);
                  setMessage({
                    type: "error",
                    text: `❌ Error: ${error.message}`,
                  });
